...
public class Ramsey {
	...
	private static void createEquals(Ramsey r) {
	...
	private static File parseParams(String[] args, Ramsey r) {
		if (args.length > 0) {
	...
	private void eqBuilder() {
		...
		for (ArrayList<Tupel> eq : equals) {
			...
			for (int i = 0; i < eq.size() - 1; i++) {
				writer.print("(iff (t" + j++ + ") (= (p" + eq.get(i).a + "_" + eq.get(i).b + ") (p" + eq.get(i + 1).a + "_"
						+ eq.get(i + 1).b + "))) ");
			}
		}
	}
